#ifndef BLAKE256_BLAKE256_H
#define BLAKE256_BLAKE256_H
...
void Blake256Compress(state256 *S, const uint8_t *block) {
...
void Blake256Update(state256 *S, uint8_t *in, uint64_t inlen) {
    int left = S->buflen;
    int fill = 64 - left;
    if (left && (inlen >= fill)) {
...
void Blake256Final(state256 *S, uint8_t *out) {
    ...
    if (S->buflen == 55) {
        S->t[0] -= 8;
        Blake256Update(S, &oo, 1);
    } else {
        if (S->buflen < 55) {
            if (!S->buflen) S->nullt = 1;
            S->t[0] -= 440 - (S->buflen << 3);
            Blake256Update(S, PADDING, 55 - S->buflen);
        } else {
            S->t[0] -= 512 - (S->buflen << 3);
            Blake256Update(S, PADDING, 64 - S->buflen);

            S->t[0] -= 440;
            Blake256Update(S, PADDING + 1, 55);
            S->nullt = 1;
        }
        Blake256Update(S, &zo, 1);
        S->t[0] -= 8;
    }
    S->t[0] -= 64;
    Blake256Update(S, msglen, 8);
...
void Blake256Hash(uint8_t *out, uint8_t *in, uint64_t inlen) {
    state256 S;
    Blake256Init(&S);
    Blake256Update(&S, in, inlen);
    Blake256Final(&S, out);
}

#endif //BLAKE256_BLAKE256_H
